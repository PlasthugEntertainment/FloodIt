...
game * CreateGame(unsigned char p_size, unsigned char p_nbColor)
{
	...
	for (size_t i = 0; i < p_size * p_size; i++)
	{
	...
	printf("coloration du board\n");
	int proba = 75;
	unsigned char buff_color = 0;
	for (unsigned char i = 0; i < p_size; i++)
	{
		if (rand() % 100 <= proba)
			result->m_board->m_graph[i]->m_colorIndex = buff_color;
		else
...
{
	unsigned char boardOutSize = 1;

	printf("recherche du nombre de noeuds du graphe réduit\n");
	// 1. Recherche du nombre de noeud du graphe réduit
	for (unsigned char i = 1; i < p_boardIn->m_size; i++)
	{
		if (p_boardIn->m_graph[i]->m_colorIndex != p_boardIn->m_graph[i - 1]->m_colorIndex)
			boardOutSize++;
	}

	(*p_boardOut)->m_size = boardOutSize;

	printf("creation du graphe réduit\n");
	// 2. Création du graphe réduit
	(*p_boardOut)->m_graph[0]->m_colorIndex = p_boardIn->m_graph[0]->m_colorIndex;

	for (unsigned char i = 1; i < p_boardIn->m_size; i++)
	{
		if (p_boardIn->m_graph[i]->m_colorIndex != p_boardIn->m_graph[i - 1]->m_colorIndex)
			(*p_boardOut)->m_graph[i]->m_colorIndex = p_boardIn->m_graph[i - 1]->m_colorIndex;
	}
...
{
	printf("vérification des boards\n");
...
void FloodFill(node * p_node, unsigned char p_targetColor, unsigned char p_replacementColor)
{
	...
	if (p_node->m_colorIndex == p_replacementColor)
		return;

	if (p_node->m_colorIndex == p_targetColor)
		return;

	p_node->m_colorIndex = p_replacementColor;
	FloodFill(p_node->m_neighbours->m_node, p_targetColor, p_replacementColor);
	FloodFill(p_node->m_neighbours->m_next->m_next, p_targetColor, p_replacementColor);
	FloodFill(p_node->m_neighbours->m_next->m_next->m_next, p_targetColor, p_replacementColor);
	FloodFill(p_node->m_neighbours->m_next->m_next->m_next->m_next, p_targetColor, p_replacementColor);
	return;
}

/**Fonction
 *	@brief	 permettant d'appliquer une couleur au plateau.
 *
...
{
	printf("recoloration du board\n");

	FloodFill((*p_board)->m_graph[0], (*p_board)->m_graph[0]->m_colorIndex, p_colorIndex);
...
{
	char buff_romain_va_niquer_ta_maman = p_board->m_graph[0]->m_colorIndex;

	for (unsigned int i = 1; i < p_board->m_size; i++)
	{
		if (p_board->m_graph[i]->m_colorIndex != buff_romain_va_niquer_ta_maman)
			return 0;
	}
